...
- [ ] **Create data service layer** (`src/services/` or similar)
  - All API calls outside React components (use Axios, already configured in `src/api.ts`)
...
- [ ] **US#10 — Extract key figures from user info**
  - `keyData.calorieCount` (format: "X,XXXkCal"), `keyData.proteinCount` (Xg), `keyData.carbohydrateCount` (Xg), `keyData.lipidCount` (Xg)

- [ ] **Mock data implementation**
  - Create local mock data mirroring API response shape (use `public/data/data.js` as reference)
  - Service should support switching between mock and real API
  - Start development with mock, switch to real API once functional

- [ ] **Data standardization/formatting**
  - Create data models/types that normalize API differences between users
